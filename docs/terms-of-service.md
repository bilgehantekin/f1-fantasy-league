# Terms of Service for GridCall

**Last Updated**: May 4, 2026  
**Effective Date**: June 1, 2026

> **Disclaimer**: GridCall is an independent, unofficial fan-made application. It is **not affiliated with, endorsed, sponsored, or associated with Formula 1, the FIA, Formula One Management, any team, driver, or sponsor**. All trademarks and logos referenced are the property of their respective owners and are used here, where they appear at all, only in a descriptive/editorial sense.

## 1. Agreement to Terms

By downloading, installing, or using GridCall ("**Service**," "**App**," or "**Application**"), you agree to be bound by these Terms of Service ("**Terms**"). If you do not agree to these Terms, do not use the Service.

**GridCall** ("**Company**," "**we**," "**us**," "**our**") reserves the right to modify these Terms at any time. Changes become effective when posted. Your continued use of the Service constitutes acceptance.

---

## 2. Description of Service

### 2.1 What is GridCall?

GridCall is an unofficial mobile application that enables users to:
- Make predictions about Formula 1 race outcomes (winner, podium, pole, DNF count, etc.)
- Create and join private leagues with friends
- Track and compare prediction scores with other users
- View Formula 1 race schedules and post-race results from publicly available sources
- Participate in seasonal competitions

**GridCall is for entertainment purposes only.** Predictions do not constitute gambling, betting, or financial transactions and there are no real-money winnings.

### 2.2 Service Availability

We provide the Service on an "AS-IS" and "AS-AVAILABLE" basis. We do not guarantee:
- Uninterrupted availability
- Error-free operation
- Compatibility with all devices

We may modify, suspend, or terminate the Service with reasonable notice.

---

## 3. User Accounts

### 3.1 Account Creation

To use GridCall, you must create an account by:
- Email signup (email + password), OR
- Sign in with Google, OR
- Sign in with Apple

### 3.2 Account Eligibility

You represent and warrant that:
- You are at least 13 years old (16 in some EU jurisdictions)
- You have authority to enter into this agreement
- You will comply with all applicable laws
- You are not prohibited from using the Service (e.g., under sanctions)

### 3.3 Account Security

You are responsible for:
- Keeping your password confidential
- All activity under your account
- Promptly notifying us of unauthorized access

We are not liable for unauthorized access due to your negligence.

### 3.4 Account Information

You agree to:
- Provide accurate, complete information
- Update information if it changes
- Not impersonate others or misrepresent identity

---

## 4. User Conduct & Prohibited Behavior

### 4.1 Prohibited Activities

You agree NOT to:

**Fraudulent/Illegal Activity**:
- Violate any law or regulation
- Engage in fraud, scams, or deceptive practices
- Solicit money or valuables under false pretenses

**Harassment & Abuse**:
- Harass, threaten, intimidate, or abuse other users
- Send unsolicited promotional messages
- Spam or flood the service
- Cyberbully or defame others

**Manipulation & Cheating**:
- Manipulate predictions or results
- Create multiple accounts to gain unfair advantage
- Exploit bugs or vulnerabilities
- Engage in any form of match-fixing or cheating

**Intellectual Property**:
- Infringe copyright, trademark, or patents
- Reverse-engineer or decompile the app
- Extract or scrape data
- Remove copyright notices

**Hacking & Security**:
- Attempt to gain unauthorized access
- Disrupt service availability
- Introduce viruses, malware, or malicious code
- Perform distributed denial-of-service (DDoS) attacks

**Data & Privacy**:
- Collect data about other users without consent
- Share personal information of others
- Violate others' privacy

### 4.2 Consequences of Violations

If you violate these Terms, we may:
- Issue a warning
- Suspend your account temporarily
- Permanently terminate your account
- Report to law enforcement if necessary
- Pursue legal action

---

## 5. League Rules & Gameplay

### 5.1 Creating & Managing Leagues

**League Owner Responsibilities**:
- You must be 18+ years old to create a league
- You are responsible for league rules and member conduct
- You can remove members at your discretion
- You can transfer league ownership or delete the league

**League Invitations**:
- Invite codes are unique per league
- Sharing the code = giving access (treat as confidential)
- Users who join agree to these Terms and the league's rules

### 5.2 Predictions & Scoring

**Prediction Submission**:
- Predictions are locked at the official race start time
- Late submissions are NOT accepted
- Edits are NOT allowed after submission
- Your predictions must be original (no cheating/collusion)

**Scoring System**:
- Scores calculated using official Formula 1 race results (OpenF1 API)
- Points awarded based on accuracy of predictions
- Joker cards (if available) double points for that prediction
- Final standings determined automatically

**Disputes**:
- Scoring is final and determined by the system
- We do not overturn scores except for technical errors
- Contact support if you believe there's a technical error

### 5.3 Joker Question

**What Is the Joker?**
- A bonus question attached to some races (e.g., "Will there be a safety car?") worth extra points if you answer correctly.
- The joker question only becomes visible inside the prediction screen during the **24 hours before prediction lock** for that race; before that window it is hidden for everyone equally.
- Joker availability and points value can vary per race and may not exist for every race.

**Usage**:
- The joker can only be answered before prediction lock.
- If you do not answer, you simply forgo the bonus points; nothing is consumed.
- Joker answers, like other predictions, can be cleared from the prediction screen until lock.

### 5.4 League Conduct

In leagues, you must:
- Treat other members with respect
- Not spam or abuse other members
- Not coordinate to gain unfair advantage
- Report abuse to league owner or us

---

## 6. Intellectual Property Rights

### 6.1 Company IP

All content in GridCall is owned by us or our licensors:
- App design, code, and functionality
- Graphics, logos, and branding
- Documentation and help content

### 6.2 User Content

By submitting predictions, comments, or league names, you grant us:
- Non-exclusive license to use your content
- Right to modify for display purposes
- Right to use for analytics and improvement

You retain ownership of your content.

### 6.3 Third-Party IP

- Formula 1® is a trademark of Formula 1 World Championship Limited
- OpenF1 API: Used under its terms
- Google, Apple: Used under their licensing agreements

---

## 7. Disclaimers & Limitations of Liability

### 7.1 NO WARRANTY

THE SERVICE IS PROVIDED "AS-IS" WITHOUT WARRANTIES OF ANY KIND:

- We do NOT warrant that the Service will be uninterrupted or error-free
- We do NOT warrant that predictions will be accurate or fair
- We do NOT warrant compatibility with all devices
- We do NOT guarantee specific results or outcomes

### 7.2 LIMITATION OF LIABILITY

**TO THE FULLEST EXTENT PERMITTED BY LAW:**

We are NOT liable for:
- Data loss or corruption
- Loss of profits or revenue
- Loss of use or interruption of service
- Indirect, incidental, special, or consequential damages
- Errors or inaccuracies in predictions or scores
- Third-party actions or content
- Your reliance on the Service

**Maximum Liability**: Our total liability shall not exceed the amount you paid for the Service (if applicable), or $1 USD if free.

### 7.3 APPLICABLE TO YOUR LOCATION

Some jurisdictions do not allow exclusion of implied warranties. In those cases, we exclude liability to the maximum extent permitted.

---

## 8. Third-Party Services

### 8.1 Integration with Third Parties

GridCall uses:
- **Google & Apple**: Sign-in services
- **OpenF1 API**: F1 race data and results
- **Supabase**: Database and authentication infrastructure
- **Firebase**: Push notifications
- **Sentry**: Error reporting

We are not responsible for:
- Third-party service outages
- Third-party changes to APIs
- Third-party privacy practices

### 8.2 Links to Third-Party Sites

GridCall may contain links to third-party websites. We are not responsible for:
- Content or accuracy of third-party sites
- Availability of third-party sites
- Third-party terms and policies

You access third-party sites at your own risk.

---

## 9. Account Deletion & Data Removal

### 9.1 Deleting Your Account

You can request account deletion anytime in the app:
Profile → Settings → Delete Account

### 9.2 What Happens When You Delete

Upon deletion:
- Your personal data (email, username, avatar) is permanently deleted
- Your predictions are permanently deleted
- You are removed from all leagues
- Your profiles are no longer accessible

**Important**: This action cannot be undone.

### 9.3 Data Retention Exception

We retain:
- Anonymous league statistics (cannot identify you)
- Audit logs of deletion (required by law)
- We do NOT retain your personal/prediction data

---

## 10. Pricing & Payment

GridCall is currently **free to download and use**. There are no in-app purchases, subscriptions, or paid tiers in this version of the app. All features described in these Terms are available to every user at no cost.

If we introduce paid features in a future version, we will update these Terms in advance and the App Store / Play Store listing will disclose the price, billing period, and cancellation/refund policy in accordance with platform rules. We will not collect payment information through the app outside of the standard platform billing flow (StoreKit / Google Play Billing).

---

## 11. Indemnification

You agree to indemnify and hold us harmless from:
- Claims by others related to your use of the Service
- Your violation of these Terms
- Your violation of any law
- Your infringement of third-party rights
- Any content you provide

---

## 12. Governing Law & Dispute Resolution

### 12.1 Governing Law

These Terms are governed by the laws of United States, without regard to conflict of law principles.

### 12.2 Dispute Resolution

**Before Litigation**:
- Contact us first: bilgehan.2002@gmail.com
- Allow 30 days for resolution attempts
- Both parties agree to negotiate in good faith

**Binding Arbitration** (if applicable in your jurisdiction):
- Any dispute shall be resolved by binding arbitration
- Arbitration held in New York
- Rules: AAA Arbitration Rules
- Costs: Each party bears own costs (except where law requires otherwise)

**Exceptions**: IP infringement claims and injunctive relief may be pursued in court.

### 12.3 Class Action Waiver

You agree NOT to pursue class actions. Disputes must be individual arbitration.

---

## 13. Severability

If any provision of these Terms is found invalid or unenforceable:
- That provision will be modified to the minimum extent necessary
- Remaining provisions remain in full force

---

## 14. Entire Agreement

These Terms (including Privacy Policy and any amendments) constitute the entire agreement between you and us regarding the Service.

If there's any conflict between these Terms and other documents, these Terms control.

---

## 15. Contact Us

### 15.1 Customer Support

For questions or support:
- **Email**: bilgehan.2002@gmail.com
- **Response Time**: 5-7 business days

### 15.2 Legal Notices

Formal legal notices can be sent to:
- **Email**: bilgehan.2002@gmail.com

Notice is effective when email is sent.

---

## 16. Additional Policies

### 16.1 Related Agreements

By using GridCall, you also agree to:
- **Privacy Policy**: https://gridcall.app/privacy
- **App Store Terms**: Apple App Store or Google Play Store terms

### 16.2 Accessibility

We are committed to accessibility. If you have accessibility questions or need accommodations, please contact us.

---

## 17. Final Provisions

### 17.1 No Waiver

Failure to enforce any right does not constitute waiver of that right.

### 17.2 Amendments

We reserve the right to amend these Terms. Changes take effect when posted. Continued use = acceptance.

### 17.3 Interpretation

- "You" / "Your" = you and any authorized representative
- "We" / "Us" = GridCall and its affiliates/partners
- Section headings are for convenience only

---

## 18. Special Notices

### 18.1 iOS & Android App Stores

- These Terms do not override App Store terms
- App Store terms may differ; use the most restrictive version
- App Store has no liability except as required by law

### 18.2 EULA Compliance

GridCall complies with:
- Apple App Store End-User License Agreement
- Google Play Terms of Service
- Industry best practices

---

**GridCall Terms of Service**  
Last Updated: May 4, 2026  
Effective: June 1, 2026

---

## Acknowledgment

**By downloading and using GridCall, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.**

If you have questions, please contact: bilgehan.2002@gmail.com

---

*This Terms of Service was created to comply with app store requirements and provide a fair agreement between GridCall and its users.*
