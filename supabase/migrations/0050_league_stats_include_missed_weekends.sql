-- 0050: Trend grafiği ve son haftalar kartlarında kullanıcının tahmin
-- yapmadığı haftalar 0 puan olarak gösterilsin. En iyi & en kötü kartları
-- ise hâlâ sadece gerçekten tahmin yapılan haftalar üzerinden hesaplanır.
...
begin
  ...
  -- Sadece kullanıcının gerçekten tahmin yaptığı haftalar (best/worst için).
  rows as (
    select s.race_id,
           s.race_name,
           s.round,
  ...
  -- Trend ve son haftalar için: yarışı biten tüm haftalar; tahmin yoksa 0.
  trend_rows as (
    select r.id as race_id,
           r.name as race_name,
           r.round,
